...
Pb_ATTACH_ATOM=[[['O1_1_0','O1_2_0']],[['O1_2_0']]]#The initial lead postion (first one) for domain1 is [0.5,0.56955,2.0]
Pb_ATTACH_ATOM_OFFSET=[[[None,None]],[[None]]]
O_NUMBER=[1,2]
...
for i in range(DOMAIN_NUMBER):
    ...
    for j in range(Pb_NUMBER[i]):
        if len(Pb_ATTACH_ATOM[i][j])==1:
            ...
            O_id=[vars()['HO_list_domain'+str(int(i+1))+'a'][HO_index-1],vars()['HO_list_domain'+str(int(i+1))+'a'][HO_index-2]]
            sorbate_coors=vars()['domain_class_'+str(int(i+1))].adding_sorbate_pyramid_monodentate(domain=vars()['domain'+str(int(i+1))+'A'],top_angle=1.,phi=0.,r=2.25,attach_atm_ids=ids,offset=offset,pb_id=pb_id,O_id=O_id)
            ...
            offset=Pb_ATTACH_ATOM_OFFSET[i][j]
            pb_id=vars()['pb_list_domain'+str(int(i+1))+'a'][j]
            O_id=[vars()['HO_list_domain'+str(int(i+1))+'a'][HO_index-1]]
...
def Sim(data,VARS=VARS):
    ...
    for i in range(DOMAIN_NUMBER):
        ...
        for j in range(Pb_NUMBER[i]):
            if len(Pb_ATTACH_ATOM[i][j])==1:
                ...
                phi=VARS['rgh_domain'+str(int(i+1))].phi
                r=VARS['rgh_domain'+str(int(i+1))].r
                HO_index=HO_index+2
                ids=[Pb_ATTACH_ATOM[i][j][0]+'_D'+str(int(i+1))+'A']
                offset=Pb_ATTACH_ATOM_OFFSET[i][j]
                pb_id=VARS['pb_list_domain'+str(int(i+1))+'a'][j]
                O_id=[VARS['HO_list_domain'+str(int(i+1))+'a'][HO_index-1],VARS['HO_list_domain'+str(int(i+1))+'a'][HO_index-2]]
                sorbate_coors=VARS['domain_class_'+str(int(i+1))].adding_sorbate_pyramid_monodentate(domain=VARS['domain'+str(int(i+1))+'A'],top_angle=top_angle,phi=phi,r=r,attach_atm_ids=ids,offset=offset,pb_id=pb_id,O_id=O_id)
                pb_coors_a.append(sorbate_coors[0])
                [O_coors_a.append(sorbate_coors[k]) for k in range(len(sorbate_coors))[1:]]
            elif len(Pb_ATTACH_ATOM[i][j])==2:
                top_angle=VARS['rgh_domain'+str(int(i+1))].top_angle
    ...
    #print domain_creator3.extract_coor(domain1A,'O1_2_0_D1A')
    #print domain_creator3.extract_coor(domain1A,'Pb1_D1A')
