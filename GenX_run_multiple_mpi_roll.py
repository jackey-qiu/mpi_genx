...
for pars in par_list:
    ...
    #set fit switch of next set of parameters
    if rank==0:
        rows=opt.parameters.get_len_rows()
        i_false=0
        for row in range(len(rows)):
            if (opt.parameters.get_value(row,2)==True):
                opt.parameters.set_value(row,2,False)
                if opt.parameters.get_value(row+1,2)==False:
                    i_false=row+1
                    break
        for row in range(i_false,i+10):
            try:
                opt.parameters.set_value(row,2,True)
            except:
                break
        opt.autosave()
