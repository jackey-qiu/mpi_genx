...
class Sample:
    ...
    def calc_f(self, h, k, l):
        ...
        for i in self.domain.keys():
            f1=lambda h,k,l,x_n,y_n,z_n:np.exp(2.*np.pi*1.0J*(h*x_n+k*y_n+l*z_n)) 
            if self.coherence==True:
                if self.domain[i]['tag']==True:
                    fs=f1(h,k,l,self.domain[i]['N_xy'][0],self.domain[i]['N_xy'][1],self.domain[i]['N_xy'][2])\
                        *(fb+self.calc_fs(h, k, l,[self.domain[i]['slab']]))*self.domain[i]['wt']
                else:fs=(fb+self.calc_fs(h, k, l,[self.domain[i]['slab']]))*self.domain[i]['wt']
                        
                ftot=ftot+fs
            else:
                fs=abs((fb+self.calc_fs(h, k, l,[self.domain[i]['slab']]))*self.domain[i]['wt'])
                ftot=ftot+fs
...
class Slab:
    ...
    def get_atm_position(self,id):
        index=np.where(self.id==id)[0][0]
        x=self.x[index]+self.dx1[index]+self.dx2[index]+self.dx3[index]+self.dx4[index]
        y=self.y[index]+self.dy1[index]+self.dy2[index]+self.dy3[index]+self.dy4[index]
        z=self.z[index]+self.dz1[index]+self.dz2[index]+self.dz3[index]+self.dz4[index]
        return [x,y,z]
    ...
    def _extract_values(self):
        if self.T_factor=='B':
            return  self.x + self.dx1+self.dx2+self.dx3+self.dx4, self.y + self.dy1+self.dy2+self.dy3+self.dy4, self.z + self.dz1+ self.dz2+ self.dz3+self.dz4,\
                    self.el, 3.*self.u/(8*np.pi**2), self.oc*self.m*self.slab_oc, self.c
