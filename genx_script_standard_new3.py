...
##pars for sorbates##
Pb_NUMBER=[0,1]#domain1 has 1 pb, sencond item represent the number of Pb in second domain
Pb_ATTACH_ATOM=[[[]],[['O1_1_0']]]#The initial lead postion (first one) for domain1 will form a bidentate mode with O1 AND O2
Pb_ATTACH_ATOM_OFFSET=[[[None]],[[None]]]#consider the unit offsets of the above two atms
O_NUMBER=[[0],[2]]#[[1,2]]means domain has two pb sorbate with one corresponding to monodentate the other one to bidentate
TOP_ANGLE=[[1.4],[1.1]]#open angel for the surface complex structure
PHI=[[1],[1]]#rotation angle for the surface complex structure
...
V_SHIFT=[2,2]#vertical shiftment of water molecules, in unit of angstrom
...
##symmetry library
#note the Fe output file is the same, but O and Pb output file is different due to different sorbate configuration
#also note the nameing rule for O pb output file
for i in range(DOMAIN_NUMBER): 
    ...
    if WATER_NUMBER[i]!=0:#add water molecules if any
        for jj in range(WATER_NUMBER[i]/2):#note will add water pair (two oxygens) each time, and you can't add single water 
            ...
            try:
                ...
                H2O_coors_a=vars()['domain_class_'+str(int(i+1))].add_oxygen_pair2(domain=vars()['domain'+str(int(i+1))+'A'],O_ids=O_ids_a,ref_id='O1_1_0_D'+str(i+1)+'A',v_shift=V_SHIFT[i],r=R[i],alpha=ALPHA[i])
